...
export default function MentionInput({
  ...
  return (
    <div className="relative" onClick={e => e.stopPropagation()}>
      {multiline ? (
        <textarea {...sharedProps} ref={inputRef as React.RefObject<HTMLTextAreaElement>} rows={rows} />
      ) : (
          ...
          >
            {loading ? (
              <div className="flex items-center justify-center py-4 gap-2 text-zinc-400 text-sm">
                <div className="h-4 w-4 border-2 border-dk-primary border-t-transparent rounded-full animate-spin" />
                Searching...
              </div>
                ...
                {suggestions.map((user, idx) => (
                  ...
                  >
                    ...
                    <img
                      src={
                        user.profilePic ||
                        `https://ui-avatars.com/api/?name=${user.username || 'User'}&background=DFE104&color=000&bold=true&size=64`
                      }
                    ...
                    {user.role && (
                      <span className="ml-auto flex-shrink-0 text-[10px] font-black uppercase tracking-wider text-dk-primary bg-dk-primary/10 px-2 py-0.5 rounded-full">
                        {user.role}
                      </span>
                    )}
                  </button>
