...
const CATS = [
  { id: 'recent', label: 'Recent', icon: '🕐', e: [] as string[] },
  {
    ...
    >
      ...
      <div className="p-3 border-b border-white/[0.06]">
        <input
          ...
          className="w-full rounded-xl bg-white/[0.06] px-3 py-2 text-sm text-white placeholder:text-zinc-500 outline-none focus:ring-1 focus:ring-dk-primary/40"
        />
      </div>
        ...
        >
          {cats.map(cat => (
            <button
              ...
              title={cat.label}
              className={`flex-shrink-0 w-8 h-8 rounded-lg text-base flex items-center justify-center transition-all ${activeCat === cat.id ? 'bg-dk-primary/20 text-dk-primary' : 'text-zinc-400 hover:bg-white/[0.06] hover:text-white'}`}
            >
              {cat.icon}
            </button>
          ))}
