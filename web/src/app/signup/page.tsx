...
export default function SignupPage() {
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update profile with username temporarily mapping to displayName
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      // Write profile (AuthProvider checks if it exists, but we overwrite/initialize here)
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-dk-bg relative overflow-hidden">
      {/* Background Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-dk-neon-blue/20 rounded-full blur-[100px] pointer-events-none" />
      ...
      <div className="w-full max-w-md glass-panel rounded-3xl p-8 relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-zinc-400">
            Create an account to discover nightlife
          </p>
        </div>
        ...
        <p className="text-center text-sm text-zinc-400 mt-8">
          ...
          <Link href="/login" className="text-dk-primary font-bold hover:underline">
            Sign In
          </Link>
        </p>
      </div>
